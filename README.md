...
- **맵: 경복궁 근정전** — 제공받은 STL 모델을 그대로 지형으로 사용합니다. 회랑으로 둘러싸인 마당 → 앞쪽 계단 → 월대 → 기단 → 전각 내부까지 실제로 걸어 올라갈 수 있습니다.
- **검과 방패를 든 기사 캐릭터** — KayKit Adventurers `Knight` 모델(CC0). 모델에 포함된 실제 애니메이션만 사용합니다.
- 고정 각도 카메라 (마우스 회전 없이 플레이어를 계속 따라다님), 지붕이 시야를 가리면 자동으로 투명해짐
- 플레이어를 따라다니는 횃불 조명

### 맵 처리 방식

STL은 면 정보만 있어서, 변환 스크립트(`tools/convert_map.py`)로 게임에 필요한 데이터를 미리 뽑아 씁니다.

1. ASCII STL(4.2MB) → 바이너리 STL(1.1MB)로 변환
2. 면의 높이와 기울기로 **석재 / 목재(단청) / 기와** 3가지 재질을 자동 분류
3. 224×224 격자에 **바닥 높이맵**을 구워냄 — 지붕(z≥45)은 제외하고 걸을 수 있는 면만
4. 원본 모델에 없는 **전각 내부 바닥을 자동으로 메움**, 앞뒤 어칸에 **출입구**를 뚫음
5. 기둥·벽 위치를 **장애물 마스크**로 저장 (비트 단위로 압축)
6. 계단만 골라 완만하게 다듬되, **월대 옆면 같은 절벽은 원본 그대로 유지**해서 뛰어내리지 못하게 함

게임에서는 이 데이터를 `src/terrain.js`가 읽어서 캐릭터가 지형을 따라 오르내리고, 기둥에 막히고,
월대에서 떨어지지 않도록 처리합니다. 적도 같은 규칙으로 움직여서 계단으로 쫓아옵니다.
