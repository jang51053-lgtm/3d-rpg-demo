...
- **검을 든 기사 캐릭터** — KayKit Adventurers `Knight` 모델(CC0). 한손검을 들고 있고, 모델에 포함된 실제 애니메이션(Idle / Running / 검 공격 / 구르기 / 사망)을 그대로 사용
- 고정 각도 카메라 (마우스 회전 없이 플레이어를 계속 따라다님)
- 체크무늬 던전 타일 바닥 + 벽/기둥으로 구획된 아레나, 플레이어를 따라다니는 횃불 조명
- WASD 이동, **구르기(Space)** — `Dodge_Forward` 애니메이션 + 부드러운 이동, 구르는 동안 무적
- **검 공격(마우스 좌클릭 / 화면 버튼)** — `1H_Melee_Attack_Slice_Diagonal` 스윙, 검이 실제로 닿는 타이밍(0.22초)에 판정 + 베기 궤적 이펙트 + 넉백 + 카메라 흔들림
- 적 4마리(붉게 물들인 기사)가 플레이어를 추격하고 근접 시 검으로 공격, 처치 시 사망 애니메이션
...
| --- | --- | --- |
| 이동 | `W` `A` `S` `D` | — |
| 공격 | **마우스 좌클릭** (또는 `X`) | 화면 우측 하단 🗡️ 버튼 |
| 구르기 | `Space` | 화면 우측 하단 🤸 버튼 |
...
- [x] 검을 든 기사 캐릭터 모델 + 전용 애니메이션(대기/달리기/공격/구르기/사망)
- [x] 구르기(회피) 및 적 추격 AI
- [x] 마우스 좌클릭 공격 + 터치용 화면 버튼
- [ ] 콤보 공격 (모델에 Chop / Slice / Stab 4종이 들어있어 연결 가능)
- [ ] 방패 막기 (`Block` 애니메이션 포함되어 있음)
